...
class Tracker:
    ...
    def interpolate_ball_position(self, ball_positions):
        ...
        # If interpolation doesn't work, use back fill (to fill the position with the closest position detected)
    ...
    def get_object_tracks(self, frames, read_from_stub=True, stub_path=None):
        ...
        # Convert it to supervision detection
            detection_supervision = sv.Detections.from_ultralytics(detection)

        # Convert the goalkeeper object to a player object
        ...
        # Assigning tracker
            track_with_detection = self.tracker.update_with_detections(detection_supervision)
            tracks["players"].append({})
            tracks["referees"].append({})
            tracks["ball"].append({})

            for frame_detection in track_with_detection:
    ...
    def draw_ball_control(self, frame, frame_num, team_ball_control):
        ...
        cv2.putText(frame, f"Team A: {team_1*100:.2f}%",
                    ...
                    (255,255,255),
                    2)
        cv2.putText(frame, f"Team B: {team_2*100:.2f}%",
                    (1400, 130),
                    cv2.FONT_HERSHEY_COMPLEX,
                    0.8,
                    (255,255,255),
