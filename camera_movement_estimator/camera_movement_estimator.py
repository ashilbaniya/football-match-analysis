...
class CameraMovementEstimator:
    ...
    def draw_camera_movement(self, frames, camera_movement_per_frame):
        ...
        for frame_num, frame in enumerate(frames):
            ...
            overlay = frame.copy()
            cv2.rectangle(overlay, (0,60), (500,140), (255,255,255), cv2.FILLED)
            alpha = 0.4
            cv2.addWeighted(overlay, alpha, frame, 1 - alpha, 0, frame)

            x_movement, y_movement = camera_movement_per_frame[frame_num]

            frame = cv2.putText(frame, f"X movement: {x_movement:.2f}", (60,90), cv2.FONT_HERSHEY_COMPLEX, 1, (0,0,0), 3)
            frame = cv2.putText(frame, f"Y movement: {y_movement:.2f}", (60,135), cv2.FONT_HERSHEY_COMPLEX, 1, (0,0,0), 3)
